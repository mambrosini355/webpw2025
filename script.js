...
document.getElementById('checkout-form').onsubmit = (e) => {
    ...
    const serviceID = 'service_0sq86bm';
    const templateID = 'template_8siuddp';

    const checkoutButton = e.target.querySelector('button[type="submit"]');
    checkoutButton.disabled = true;
    checkoutButton.textContent = 'Enviando...';

    const orderDetails = carrito.map(item => {
        return `${item.nombre} (x${item.cantidad}) - $${formatearPrecio(item.precio * item.cantidad)}`;
    }).join('\n');
    
    const orderTotal = carrito.reduce((acc, p) => acc + (p.precio * p.cantidad), 0);

    const params = {
        order_id: Date.now(),
        user_name: document.getElementById('checkout-name').value,
        email: document.getElementById('checkout-email').value,
        order_details: orderDetails,
        order_total: formatearPrecio(orderTotal),
    };

    emailjs.send(serviceID, templateID, params)
        .then(() => {
            alert('¡Gracias por tu compra! Revisa tu email para ver el detalle.');
            carrito = [];
            guardarCarrito();
            actualizarCarrito();
            checkoutModal.style.display = 'none';
            checkoutButton.disabled = false;
            checkoutButton.textContent = 'Completar Compra';
            document.getElementById('checkout-form').reset();
        })
        .catch((err) => {
            alert('Ocurrió un error al enviar el email. Por favor, inténtalo de nuevo.');
